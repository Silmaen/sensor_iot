...
void test_payload_builder_single_float(void) {
    ...
    PayloadBuilder pb{buf, sizeof(buf), 0, true};
    pb.begin();
    pb.add_float("temperature", 22.5f, 1);
    pb.end();
    TEST_ASSERT_EQUAL_STRING("{\"temperature\":22.5}", buf);
}
...
void test_payload_builder_single_uint(void) {
    ...
    PayloadBuilder pb{buf, sizeof(buf), 0, true};
    pb.begin();
    pb.add_uint("battery_pct", 75);
    pb.end();
    TEST_ASSERT_EQUAL_STRING("{\"battery_pct\":75}", buf);
}
...
void test_payload_builder_multiple_fields(void) {
    ...
    pb.begin();
    pb.add_float("temperature", 22.5f, 1);
    pb.add_float("humidity", 45.2f, 1);
    pb.add_uint("battery_pct", 80);
    pb.end();
